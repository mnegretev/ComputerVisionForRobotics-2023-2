...
def hough_lines(img_bin, d_min, d_max, d_res, theta_min, theta_max, theta_res, threshold):
    dist_n = int(np.ceil((d_max - d_min) / d_res))
    theta_n = int(np.ceil((theta_max - theta_min) / theta_res))
    votes = np.zeros((dist_n, theta_n), dtype=np.uint64)
        ...
        for j in range(img_bin.shape[1]):
            if img_bin[i, j] != 0:
                for k in range(theta_n):
                    d = int((j * np.cos(theta_values[k]) + i * np.sin(theta_values[k]) - d_min) / d_res)
                    if 0 <= d < dist_n:
                        votes[d, k] += 1

    lines = []
    for d in range(dist_n):
        for k in range(theta_n):
            if votes[d, k] >= threshold:
                rho = d * d_res + d_min
...
def main():
    ...
    cv2.createTrackbar('Hough Threshold', 'Control', 10, 25, nothing)
    ...
    while True:
        ...
        hough_trackbar = cv2.getTrackbarPos('Hough Threshold', 'Control')
        
        hough_threshold = int(hough_trackbar * (100 / 25))  # Convertir el valor de la Trackbar al rango de hough_threshold
        ...
        lines = hough_lines(hysteresis_edges, 0, math.sqrt(hysteresis_edges.shape[0] ** 2 + hysteresis_edges.shape[1] ** 2),
                            1, -np.pi / 2, np.pi / 2, np.pi / 180, hough_threshold)

        # Dibujar las líneas detectadas sobre la imagen original
        hough_frame = resized_frame.copy()
        draw_lines(hough_frame, lines)

        # Mostrar las imágenes
        cv2.imshow('Original', resized_frame)
        canny_result = cv2.convertScaleAbs(hysteresis_edges)
        cv2.imshow('Canny Result', canny_result)
        cv2.imshow('Hough', hough_frame)
